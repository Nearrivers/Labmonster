<template>
  ...
  >
    ...
    <ToolbarButton @click="emit('edit')">
      <template #icon><Blocks class="h-5 w-5" /> </template>
      <template #tooltip> Ajouter une section </template>
    </ToolbarButton>
  </div>
</template>

<script setup lang="ts">
import { Blocks, ScanSearch, SquarePen, Trash2 } from 'lucide-vue-next';
